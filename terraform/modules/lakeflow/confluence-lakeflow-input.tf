# Job Definition
resource "aws_batch_job_definition" "generate_batch_jd_lakeflow_input" {
  name = "${var.prefix}-lakeflow-input"
  type = "container"

  container_properties = jsonencode({
    image            = "${local.account_id}.dkr.ecr.us-west-2.amazonaws.com/${var.prefix}-lakeflow-input:${var.image_tag}"
  ...
  propagate_tags        = true
  tags                  = { "job_definition" : "${var.prefix}-lakeflow-input" }
}
